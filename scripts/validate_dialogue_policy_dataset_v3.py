...
TRAIN_PATH = ROOT_DIR / "data" / "processed" / "dialogue_policy_train_v3.jsonl"
VAL_PATH = ROOT_DIR / "data" / "processed" / "dialogue_policy_val_v3.jsonl"
...
def validate_policy_output(output):
    errors = []

    if not isinstance(output, dict):
        return ["expected_output is not a dict"]

    intent = output.get("intent")
    action = output.get("action")
    slots = output.get("slots")
    should_update_state = output.get("should_update_state")
    should_recommend = output.get("should_recommend")
    ...
    if not isinstance(slots, dict):
        errors.append("slots is not a dict")
    else:
        missing_slots = REQUIRED_SLOT_KEYS - set(slots.keys())

        if missing_slots:
            errors.append(f"missing slot keys: {sorted(missing_slots)}")

        for key in LIST_SLOT_KEYS:
            if key in slots and not isinstance(slots[key], list):
                errors.append(f"slot {key} must be list, got {type(slots[key]).__name__}")

    if not isinstance(should_update_state, bool):
        errors.append("should_update_state must be bool")

    if not isinstance(should_recommend, bool):
...
def validate_record(item):
    ...
    if "<|im_start|>system" not in text:
        errors.append("text missing system message")

    if "<|im_start|>user" not in text:
        errors.append("text missing user message")

    if "<|im_start|>assistant" not in text:
        errors.append("text missing assistant message")

    expected_output = extract_expected_output(item)

    if expected_output is None:
        errors.append("cannot extract expected_output")
    else:
        errors.extend(validate_policy_output(expected_output))
...
def validate_file(path):
    ...
    for row in rows:
        ...
        action = expected_output.get("action")
...
def print_report(report):
    ...
    if report["invalid_items"]:
        ...
        if len(report["invalid_items"]) > 20:
            print(f"... and {len(report['invalid_items']) - 20} more invalid items")
...
def main():
    ...
    total_invalid = sum(report["invalid"] for report in reports)
    ...
    if total_invalid == 0:
        ...
        print(f"Dataset has {total_invalid} invalid records. Please fix before training.")
